  ...
  function updateView() {
    ...
    if (libraries.length === 0) {
      ...
      libraries.forEach(function(lib) {
        ...
        var name = window.ztEscHtml((lib.name || lib.Name || id));
        ...
        html += '<a class="' + active + '" href="#" data-library-id="' + window.ztEscHtml(id) + '">' + name + badge + '</a>';
      });
    }
    html += '</nav></div>';
    ...
    for (var i = 0; i < pathArr.length; i++) {
      prog += "/" + pathArr[i];
      bc += '<li class="breadcrumb-item' + (i === pathArr.length - 1 ? " active" : " br-link") + '" data-path="' + window.ztEscHtml(prog) + '">' + window.ztEscHtml(pathArr[i]) + "</li>";
    }
    bc += "</ol>";
    pathEl.innerHTML = bc;
          ...
          .then(function(r) {
            return r.json().catch(function() { return {}; }).then(function(d) {
              ...
              if (r.ok) {
                ...
                if (d.redirect) msg += ' <a href="' + window.ztEscHtml(window.ztSafeUrl(d.redirect)) + '" target="_blank">View</a>';
                if (typeof sendToast === "function") sendToast("Assign image", "", "bg-success", msg, !!d.redirect);
    ...
    function showFileDetails(fileName, filePath, fileType, fileSize, fileDate) {
      ...
      var content = '<h6>File details</h6><hr /><p><b>Name</b><br/>' + window.ztEscHtml(fileName) + '</p>';
      content += '<p><b>Type</b><br/>' + (niceTypes[fileType] || fileType) + '</p>';
      content += '<p><b>Size</b><br/>' + window.ztEscHtml(fileSize) + '</p>';
      content += '<p><b>Last modified</b><br/>' + window.ztEscHtml(fileDate) + '</p>';
      if (fileType === "video") {
        content += '<h6 class="mt-3">Preview</h6><hr /><video controls class="w-100" src="' + window.ztEscHtml(previewUrl(filePath)) + '"></video>';
      } else if (fileType === "image") {
        content += '<h6 class="mt-3">Preview</h6><hr /><img class="w-100" src="' + window.ztEscHtml(previewUrl(filePath)) + '" alt="">';
      }
      ...
      content += '<a class="btn btn-outline-primary me-2" href="' + window.ztEscHtml(previewUrl(filePath)) + '" target="_blank"><i class="fas fa-download"></i> Download</a>';
    ...
    self.querySelector("#zt-mass-action-btn").onclick = function() {
      ...
      rows.forEach(function(tr) {
        tbody.innerHTML += "<tr><td>" + window.ztEscHtml(tr.dataset.filepath) + "</td><td>" + (tr.dataset.filetype || "") + "</td></tr>";
      });
    ...
    self.querySelector("#zt-mass-import-btn").onclick = function() {
      ...
      Promise.all([
        ...
        channels.forEach(function(ch) {
          chSel.innerHTML += '<option value="' + window.ztEscHtml(ch.ID || ch.id) + '">' + window.ztEscHtml(ch.Name || ch.name) + "</option>";
        });
        var actDiv = self.querySelector("#zt-mass-actors");
        actDiv.innerHTML = "";
        actors.forEach(function(a) {
          var id = a.ID || a.id;
          actDiv.innerHTML += '<div class="form-check"><input class="form-check-input zt-mass-actor" type="checkbox" value="' + window.ztEscHtml(id) + '"><label class="form-check-label">' + window.ztEscHtml(a.Name || a.name) + "</label></div>";
        });
        ...
        categories.forEach(function(c) {
          (c.Sub || c.sub || []).forEach(function(s) {
            var sid = s.ID || s.id;
            catDiv.innerHTML += '<div class="form-check"><input class="form-check-input zt-mass-cat" type="checkbox" value="' + window.ztEscHtml(sid) + '"><label class="form-check-label">' + window.ztEscHtml(s.Name || s.name) + "</label></div>";
          });
        });
    ...
    function loadScanMetadata() {
      return Promise.all([
        ...
        actors.forEach(function(a) {
          var id = a.ID || a.id;
          actDiv.innerHTML += '<div class="form-check"><input class="form-check-input zt-scan-actor" type="checkbox" value="' + window.ztEscHtml(id) + '"><label class="form-check-label">' + window.ztEscHtml(a.Name || a.name) + "</label></div>";
        });
        ...
        categories.forEach(function(c) {
          (c.Sub || c.sub || []).forEach(function(s) {
            var sid = s.ID || s.id;
            catDiv.innerHTML += '<div class="form-check"><input class="form-check-input zt-scan-cat" type="checkbox" value="' + window.ztEscHtml(sid) + '"><label class="form-check-label">' + window.ztEscHtml(s.Name || s.name) + "</label></div>";
          });
        });
      });
    }
          ...
          .then(function(r) {
            if (r.ok) {
              return r.json().then(function(data) {
                ...
                if (typeof sendToast === "function") sendToast("Import a video", "", "bg-success", vid ? '<a href="/video/' + encodeURIComponent(vid) + '/edit" target="_blank">You can edit the video more here</a>' : "Import started", !!vid);
              });
            }
            return r.json().catch(function(){return{};}).then(function(d) {
        ...
        fetch("/api/upload/triage/import-photoset", { method: "POST", credentials: "same-origin", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) })
          ...
          .then(function(res) {
            ...
            if (res.ok) {
              ...
              if (typeof sendToast === "function") sendToast("Import photoset", "", "bg-success", psId ? 'Queued. <a href="/photoset/' + encodeURIComponent(psId) + '/edit" target="_blank">Edit photoset</a>' : "Import queued.", !!psId);
    ...
    Promise.all([
      ...
      Object.keys(folderData).forEach(function(name) {
        var safe = window.ztEscHtml(name);
      ...
      Object.keys(filesData).forEach(function(name) {
          ...
          var safeName = window.ztEscHtml(name);
          var safePath = window.ztEscHtml(filePath);
