...
ZtPlaylistView.prototype.connectedCallback = function() {
  ...
  fetch("/api/playlists/" + encodeURIComponent(id), { credentials: "same-origin" })
    ...
    .then(function(data) {
      ...
      var isVirtual = !!pl.virtual || (id && id.indexOf("unseen-") === 0);
      ...
      html += '<p class="text-muted"><a href="/profile/playlists">Your playlists</a></p>';
      if (isVirtual) html += '<p class="text-muted small">Videos you have not played yet. This list updates automatically.</p>';
      html += '<hr /></div>';
      ...
      if (!isVirtual) {
        ...
        html += '<span id="zt-playlist-rename-error" class="text-danger small" style="display:none"></span>';
      }
      html += '</div>';
      html += '<div id="zt-playlist-videos" class="row">';
      if (videos.length === 0) {
        html += '<p class="text-muted">' + (isVirtual ? 'No unseen items right now.' : 'No videos in this playlist yet. Add videos from a video or clip page.') + '</p>';
      } else {
      ...
      var renameForm = self.querySelector("#zt-playlist-rename-form");
      ...
      var deleteBtn = self.querySelector("#zt-playlist-delete-btn");
      ...
      if (!isVirtual) self.querySelectorAll(".zt-playlist-remove-video").forEach(function(btn) {
