...
ZtPhotosetView.prototype.connectedCallback = function() {
  ...
  fetch("/api/photoset/" + encodeURIComponent(id), { credentials: "same-origin" })
    ...
    .then(function(data) {
      ...
      var channelHtml = channel ? '<a class="btn btn-sm btn-dark" href="/channel/'+esc(channel.ID||channel.id)+'">' + esc(channel.Name||channel.name||"") + '</a>' : "";
      var actors = ps.Actors || ps.actors || [];
      var actorsHtml = actors.map(function(a) {
        return '<a class="btn btn-sm btn-danger me-1" href="/actor/'+esc(a.ID||a.id)+'">' + esc(a.Name||a.name||"") + '</a>';
      }).join("");
      var catsHtml = Object.keys(cats).map(function(cid) {
        return '<a class="btn btn-sm btn-secondary me-1" href="/category/'+esc(cid)+'">' + esc(cats[cid]) + '</a>';
      }).join("");
      var html = '<div><h3 class="mt-3">' + name + '</h3>';
      html += '<div class="mb-3 d-flex flex-wrap gap-1">' + channelHtml + actorsHtml + catsHtml;
      if (admin) html += ' <a class="btn btn-sm btn-outline-dark" href="/photoset/'+esc(id)+'/edit"><i class="fa fa-edit"></i> Edit</a>';
      ...
      photos.forEach(function(pv, idx) {
        ...
        html += '<img class="img-fluid rounded lazy" data-src="'+esc(thumb)+'" alt="'+esc(p.Filename||p.filename||"")+'" style="width:100%;aspect-ratio:1;object-fit:cover">';
        html += '</a></div>';
      });
      html += '</div></div>';
      self.innerHTML = html;
