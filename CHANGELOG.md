...
This new version will embark many new major features, like photosets, playlists, custom organization and directory scanning.

This version also mark (with a breaking change) the beginning of a new dedicated storage: the metadata storage.
To migrate to this new version, you will have to set of parameters to define where Zobtube will store its metadata.
It will need to be defined at start and will be mandatory for all the upcoming versions.
It can be set with `ZT_METADATA_TYPE` (`filesystem` or `s3`) and `ZT_METADATA_PATH` (details in the CLI with `--help`).
The previous parameter `ZT_MEDIA_PATH` will now be less mandatory as it only reset the path of the default library.
Once set, you can launch the metadata migration under the administration panel in `/adm/metadata-storage`.

The new "custom organization" feature bring the possibility to choose how Zobtube will rename and move your files.
It also include the possibility to totally disable any removing or renaming.

The "directory scanning" feature allows you to import all videos present in a folder recursively.
You can choose all the usual links on-the-fly, like actors, categories and channels.
You can also specify how videos should be considered (between videos, movies and clips) base on their size.

The "photoset" feature allow you to create albums of photos (with also all the usual links).
For now, it only supports importing it as a ZIP archive.
More functionalities will come later.

Finally, "playlists" allows you to create personal list of videos.
Some default playlists will be created for you with the videos you haven't seen yet.

That's all for this release!
Next release will see a major refacto of the documentation to ease onboarding new users.

If you encounter any bug, feel free to drop an issue on the [Github issue page](https://github.com/zobtube/zobtube/issues).

As usual, it is available on the [Github release page](https://github.com/zobtube/zobtube/releases/tag/0.6.0).
